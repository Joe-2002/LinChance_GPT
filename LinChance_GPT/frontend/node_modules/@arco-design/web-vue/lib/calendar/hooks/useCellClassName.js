...
function getDateValue(date2, index) {
  if (!date2) {
    return void 0;
  }
  if (is.isArray(date2)) {
...
function useClassName({
  ...
  return function getCellClassName(cellDateObj, disabled) {
    ...
    let isToday = isSameTime(cellDateObj.time, date.getNow());
    if (!panel && innerMode === "year") {
      isToday = date.getNow().isSame(cellDateObj.time, "date");
    }
    return [
      `${prefixCls}-cell`,
      {
